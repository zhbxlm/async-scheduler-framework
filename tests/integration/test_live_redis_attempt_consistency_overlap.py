...
@pytest.mark.asyncio
async def test_live_redis_completion_does_not_override_abandoned_attempt() -> None:
    ...
    try:
        ...
        async with get_session() as session:
            task = await TaskRepository.create(session, TaskCreate(name="live-attempt-overlap", payload={}))
            ...
            attempt = await ExecutionAttemptRepository.create(
                ...
                ExecutionAttemptCreate(
                    ...
                    retry_index=0,
                    lease_token="lease-live-attempt",
                ),
            )
        ...
        await worker_registry.register(WorkerInfo(worker_id="worker-live-attempt", name="attempt"))
        lease = await lock_backend.acquire(f"task:{task.id}", ttl=1)
        assert lease is not None
        
        # Make the lease and worker view stale so reconciler wins first.
        await worker_registry.deregister("worker-live-attempt")

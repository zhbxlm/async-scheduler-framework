...
@pytest.mark.asyncio
async def test_live_redis_duplicate_completion_and_reconciler_overlap_converges() -> None:
    ...
    try:
        ...
        completion_a = TaskCompletionNode(callback_dispatcher=dispatcher_a, dedup_backend=dedup)
        completion_b = TaskCompletionNode(callback_dispatcher=dispatcher_b, dedup_backend=dedup)
        ...
        async with get_session() as session:
            task = await TaskRepository.create(
                session,
                TaskCreate(name="live-duplicate-overlap", payload={}, callback_url="https://callback/live"),
            )
            ...
            attempt = await ExecutionAttemptRepository.create(
                ...
                ExecutionAttemptCreate(
                    task_id=task.id,
                    worker_id="worker-live-dup",
                    retry_index=0,
                    lease_token="lease-live-dup",
                ),
            )
        ...
        await worker_registry.register(WorkerInfo(worker_id="worker-live-dup", name="dup"))
        lease = await lock_backend.acquire(f"task:{task.id}", ttl=30)
        assert lease is not None

        results = await asyncio.gather(
            completion_a.finalize(
                runtime_task,
                TaskStatus.SUCCESS,
                result={"winner": "a"},
                finalize_latest_attempt=True,
            ),
            completion_b.finalize(
                runtime_task,
                TaskStatus.SUCCESS,
                result={"winner": "b"},
                finalize_latest_attempt=True,
        ...
        repair_count = results[2]
        dispatch_count = len(dispatcher_a.calls) + len(dispatcher_b.calls)

        assert repair_count == 0
        assert dispatch_count == 1
        assert final_task is not None
        assert final_task.status == TaskStatus.SUCCESS
        assert final_task.result in ({"winner": "a"}, {"winner": "b"})

...
@pytest.mark.asyncio
async def test_live_redis_lease_loss_and_terminal_completion_race_converges() -> None:
    ...
    try:
        ...
        async with get_session() as session:
            task = await TaskRepository.create(session, TaskCreate(name="live-lease-race", payload={}))
            ...
            attempt = await ExecutionAttemptRepository.create(
                ...
                ExecutionAttemptCreate(
                    task_id=task.id,
                    worker_id="worker-live-race",
                    retry_index=0,
                    lease_token="lease-live-race",
                ),
            )
        ...
        await worker_registry.register(WorkerInfo(worker_id="worker-live-race", name="race"))
        ...
        repair_count = results[1]

        assert final_task is not None
        assert final_task.status in {TaskStatus.SUCCESS, TaskStatus.QUEUED}

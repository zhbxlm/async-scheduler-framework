...
@pytest.mark.asyncio
async def test_live_redis_delayed_promotion_multi_worker_race() -> None:
    ...
    try:
        ...
        dequeue_a, dequeue_b = await asyncio.gather(
            queue_manager.dequeue(),
            queue_manager.dequeue(),
        )

        dequeue_tasks = [t for t in (dequeue_a, dequeue_b) if t is not None]
        assert len(dequeue_tasks) == 1
        candidate = dequeue_tasks[0]
        ...
        result_a, result_b = await asyncio.gather(
            winner.finalize(candidate, TaskStatus.SUCCESS, result={"winner": "delayed"}),
            loser.finalize(candidate, TaskStatus.SUCCESS, result={"loser": "delayed"}),
        )

        stored_result = result_a or result_b
        assert stored_result is not None
        assert stored_result.status == TaskStatus.SUCCESS
        assert stored_result.result == {"winner": "delayed"}
    finally:
        await client.flushdb()
        await client.aclose()
"""AsyncCommandProxy — Sidecar proxy for async command execution.

Executes local shell/Python commands via subprocess,
...
class AsyncCommandProxy:
    ...
    def __init__(self, endpoint: str = ""):
        """Initialize AsyncCommandProxy.

        Parameters
        ----------
        endpoint : str
            HTTP endpoint (not used in programmatic mode).
        """
    ...
    def submit(
        ...
        -------
        dict
            Dictionary with job_id and result_key.
        """
        job_id = str(uuid.uuid4())
        result_key = _result_key(job_id)
    ...
    def blpop_result(self, result_key: str, timeout: int = DEFAULT_TIMEOUT) -> dict | None:
        ...
        -------
        dict | None
            Parsed result dict, or None if timeout.
        """

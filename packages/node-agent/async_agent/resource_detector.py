...
class _Cache:
    """Simple TTL cache for resource detection results."""
...
class ResourceDetector:
    """Hardware resource detector with caching."""
    ...
    def detect(self, ray_node_state: Any = None) -> dict:
        ...
        Parameters
        ----------
        ray_node_state : Any | None
            Optional Ray node state for syncing resources.

        Returns
        -------
        dict
            Dict with keys: cpu_count, total_memory_mb, available_memory_mb, gpus.
        """
    ...
    def _detect_memory(self) -> dict:
        """Detect memory info (partial cache)."""
    ...
    def _detect_gpus(self) -> list:
        """Detect GPU info using nvidia-smi (cached)."""

"""AsyncProxyWorker — Plug-and-play Worker base class for long-running services.
...
class AsyncProxyWorker:
    ...
    def __init__(self, capability: str = "", config: dict | None = None) -> None:
        ...
        ----------
        capability : str
            Capability identifier for this worker.
        config : dict | None
    ...
    def call(self, data: Any) -> dict:
        ...
        ------
        ValueError
            If backend_path is not set.
        ProxySubmitError
            If /submit call fails.
        ProxyTimeoutError
            If BLPOP times out.
        ProxyBackendError
    ...
    async def health_check(self) -> dict:
        ...
        try:
            # Inline HTTP GET helper (from src.common.http_client)
            import httpx
            async with httpx.AsyncClient() as client:
                resp = await client.get(self._proxy_url.rstrip("/") + "/health", timeout=5.0)
                resp.raise_for_status()
            proxy_ok = True
        except Exception:
            pass

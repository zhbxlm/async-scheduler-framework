...
from src.api.routes.ops_shared import logger, router
        ...
        if is_super_admin:
            ...
            if cap_reg is not None:
                try:
                    ...
                    tenant_cap_ids = {
                        c.get("capability_id", c) if isinstance(c, dict) else c for c in tenant_caps
                    }
                    caps = [c for c in all_caps if c in tenant_cap_ids]
                except Exception:
                    caps = all_caps
                    logger.warning(
                        "capability_registry.list failed for tenant %s, showing all", tenant_id
                    )
            else:
                caps = all_caps
...
async def queue_snapshot(capability: str, request: Request, _auth: dict = Depends(authenticate)) -> dict:
    qm = getattr(request.app.state, "queue_manager", None)
    if qm is None:
        raise HTTPException(status_code=503, detail="QueueManager not initialised")

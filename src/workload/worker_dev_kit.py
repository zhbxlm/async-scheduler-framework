...
def run_local_test(
    ...
    Args:
        worker_class: The Worker class to test (must implement run()).
        input_data:   Input payload dict passed to run().
        config:       Optional init config dict passed to __init__.
        capability:   Capability name string for __init__ (default "local_test").

    Returns:
        The return value of worker.run(input_data).

    Raises:
        AttributeError: if worker_class does not implement run().
    """
...
class WorkerDevKit:
    ...
    def mock_capability(self, name: str, handler: Callable) -> None:
        ...
        Args:
            name:    Capability name (e.g. "cap_preprocess").
            handler: Callable(input_data) → output_data.
        """
    ...
    def assert_output(
        ...
        *,
        ...
        Args:
            result:        The output from worker.run().
            expected_keys: List of required top-level keys.
            allow_extra:   If False, raise if result contains unexpected keys.

        Raises:
            AssertionError: on missing or unexpected keys.
        """

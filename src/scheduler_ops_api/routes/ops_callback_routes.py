...
async def callback_summary(db_factory=DbFactory, _auth: dict = Depends(authenticate)) -> dict:
...
async def callback_dead_letters(db_factory=DbFactory, _auth: dict = Depends(authenticate), limit: int = 100) -> dict:
...
async def ack_dead_letter(outbox_id: int, db_factory=DbFactory, _auth: dict = Depends(authenticate), reason: str | None = None) -> dict:
...
async def replay_dead_letter(outbox_id: int, db_factory=DbFactory, _auth: dict = Depends(authenticate), reason: str | None = None) -> dict:
    actor = resolve_tenant_id(_auth, fallback="operator")

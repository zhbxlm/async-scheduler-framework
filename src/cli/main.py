...
import os
import click
from src.cli.client import RayAmuClient


@click.group()
@click.option("--api-url", envvar="RAY_AMU_API_URL", default="http://localhost:8000", help="Ray AMU API URL")
@click.pass_context
def cli(ctx, api_url):
    ctx.ensure_object(dict)
    ctx.obj["client"] = RayAmuClient(base_url=api_url)

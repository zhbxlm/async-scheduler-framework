...
Worker 开发是构建 Ray AMU 平台 AI 能力的核心环节。开发者可以通过两种主要方式创建 Worker：零依赖自定义 Worker 和基于 AsyncProxyWorker 的长耗时服务 Worker。这两种方式都遵循最小合约原则，开发者无需深入了解框架内部实现，只需关注业务逻辑本身。本文档将详细介绍 Worker 的开发模式、配置机制、部署方式以及完整的示例代码。

...
AsyncServiceProxy 可以通过两种方式启动：使用命令行工具 `ray-amu-proxy` 或直接运行 Python 模块。启动时会记录关键配置信息（后端 URL、Redis URL、端口、线程数），便于运维监控。
...
AsyncCommandProxy 可以通过两种方式启动：使用命令行工具 `ray-amu-async-command-proxy` 或直接运行 Python 模块。

```bash
# 方式 1：使用命令行工具
ray-amu-async-command-proxy

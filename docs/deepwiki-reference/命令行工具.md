...
Ray AMU 提供了一个功能完整的命令行工具（CLI），采用 kubectl 风格的命令结构设计，用于管理 Ray AMU 平台的各个方面。该 CLI 工具允许用户通过命令行界面执行集群管理、能力注册、任务提交、节点操作、调度配置、队列监控、租户管理、DAG 工作流编排以及 Worker 开发测试等操作。所有命令都通过统一的 HTTP 客户端与 Ray AMU API 服务进行交互，支持 JSON 格式的响应输出，便于与其他工具集成。
...
Ray AMU CLI 是一个 kubectl 风格的管理工具，通过 `ray-amu` 命令提供统一的入口点。该工具支持通过命令行参数 `--api-url` 或环境变量 `RAY_AMU_API_URL` 配置 API 服务器地址，默认值为 `http://127.0.0.1:8000`。CLI 使用 Click 框架实现命令分组和参数解析，所有命令的输出都采用 JSON 格式，便于脚本处理和日志记录。
...
```bash
ray-amu cluster get cluster-prod-01
```

将集群状态设置为 offline（下线）：

```bash
ray-amu cluster remove cluster-prod-01
```
...
```bash
ray-amu capability list
```

查看特定能力的配置详情：

```bash
ray-amu capability get image-generation
```

删除不再需要的能力：

```bash
ray-amu capability remove image-generation
```
...
```bash
ray-amu task get <task_id>
```

获取任务执行结果：

```bash
ray-amu task result <task_id>
```

取消正在执行的任务：

```bash
ray-amu task cancel <task_id>
```
...
```bash
ray-amu node list --state idle
```

为集群分配指定数量的 GPU 节点：

```bash
ray-amu node allocate \
  --cluster-id cluster-prod-01 \
  --gpus 4 \
  --gpu-type A100
```
...
```bash
ray-amu schedule list
```

启用或禁用特定调度：

```bash
ray-amu schedule toggle <schedule_id> --enabled
ray-amu schedule toggle <schedule_id> --disabled
```

删除调度：

```bash
ray-amu schedule delete <schedule_id>
```
...
```bash
ray-amu deploy list --node node-001
```

从节点卸载能力包：

```bash
ray-amu deploy undeploy \
  --node node-001 \
  --name image-generation
```
...
```bash
ray-amu tenant gen-key tenant-001
```

查看租户资源用量：

```bash
ray-amu tenant usage tenant-001
```

删除租户：

```bash
ray-amu tenant delete tenant-001
```
...
```bash
ray-amu dag validate config/dags/01_pipeline_linear.yaml
```

远程注册 DAG YAML 文件：

```bash
ray-amu dag register config/dags/01_pipeline_linear.yaml \
  --task-type image-gen \
  --task-type video-gen
```
...
```bash
ray-amu dag get pipeline-linear
```
...
```bash
ray-amu dag remove pipeline-linear
```
...
```bash
ray-amu worker test examples.custom_worker_example.IDCardRecognitionWorker \
  --validate-only
```
...
```bash
ray-amu --api-url http://api.example.com:8000 cluster list
```

### 环境变量

设置 `RAY_AMU_API_URL` 环境变量，所有命令将自动使用该地址：

```bash
export RAY_AMU_API_URL=http://api.example.com:8000
ray-amu cluster list
```

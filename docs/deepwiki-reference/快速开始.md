...
本文档旨在帮助开发者快速部署和运行 Ray AMU 异步调度框架。Ray AMU 是基于 Ray 构建的 AI 异步调度框架，支持 DAG 编排、任务执行、节点管理、Cron 调度等核心功能。通过本文档，您将了解环境要求、安装步骤、基础配置以及如何启动各个核心服务。
...
- `ray-amu-api` - 启动运维 API 服务（ops-api）
- `ray-amu-task-api` - 启动任务 API 服务（task-api）
- `ray-amu-agent` - 启动节点代理服务
- `ray-amu-proxy` - 启动异步代理服务
- `ray-amu` - 命令行管理工具
...
Ray AMU 的配置采用三层优先级：**环境变量 > 配置文件 > 代码默认值**。最小化配置需要设置 Redis 和 MySQL 连接信息。
...
```bash
# 方式 1：使用 BNS（百度内部环境）
export REDIS_BNS=group.bdrp-ray-amu-proxy.redis.all
...
```bash
export AGENT_CONFIG_FILE=/etc/ray-amu/agent.yaml
```
...
Ray AMU 由多个独立的服务组件组成，可以根据需要启动全部或部分服务。
...
# 提交任务
ray-amu task submit --dag-id my_dag --input '{"key": "value"}'
```

## 部署架构概览

下图展示了 Ray AMU 核心服务组件之间的关系和数据流。系统由 ops-api、task-api、agent-server 和 async-proxy 四个主要服务组成，通过 Redis 和 MySQL 进行协调和数据持久化。
